...
- (void)setupUI {
    ...
    [self.pickerView selectRow:(_year - _yearLeast -20 ) inComponent:0 animated:NO];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view
{
    ...
    NSString *text;
    if (component == 0) {
        text =  [NSString stringWithFormat:@"%d 年", row + _yearLeast+20];
    }else if (component == 1){
        text =  [NSString stringWithFormat:@"%d 月", row + 1];
    }else{
        text = [NSString stringWithFormat:@"%d 日", row + 1];
    }
